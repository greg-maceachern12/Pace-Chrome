(function() {
    ...
    function determineOriginalUnit(element) {
      ...
      if (abbr) {
        ...
        // Check for pace units
        if (title.includes('minute')) {
          return {
            type: 'pace',
        ...
        if (title.includes('mile') || title.includes('kilometer')) {
          return {
            type: 'distance',
            isMetric: text.includes('km')
          };
      ...
      // If no abbr tag, check the text content
      const textContent = element.textContent.toLowerCase();
      if (textContent.includes('/km') || textContent.includes('/mi')) {
        return {
          type: 'pace',
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      const hasRelevantChanges = mutations.some(mutation => {
        return Array.from(mutation.addedNodes).some(node => {
          ...
          // Check for any of our target selectors
          return node.matches('.------packages-ui-Stat-Stat-module__statValue--phtGK') ||
